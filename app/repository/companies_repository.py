...
class CompaniesRepository:
    ...
    async def company_get(self,company_id,session):
        res = await session.execute(select(CompanyModel).where(CompanyModel.id == company_id))
    ...
    async def create_company(self, db: AsyncSession, name: str, description: str, is_public: bool):
    ...
    async def add_user_role(self, db: AsyncSession, user_id: UUID, company_id: UUID, role: str):
    ...
    async def delete_company(self,db:AsyncSession,company_id:UUID,user:UserModel):
        res = await db.execute(select(CompanyModel).join(CompanyUserRoleModel).where(CompanyModel.id == company_id,CompanyUserRoleModel.user_id == user.id,  CompanyUserRoleModel.role == "owner",))
